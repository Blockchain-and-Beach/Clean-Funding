...
organizationSchema.statics.findByNickname = async name=>{
...
organizationSchema.statics.validate = organization=>{
    return !findByID(organization.ID) && !findByNickname(organization.name) && !findByPublicKey(organization.publicKey);
};
